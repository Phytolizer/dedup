...
SlurpFileResult slurp_file(str filename)
{
    ...
    if (!f.valid)
    {
        str msg = str_fmt("failed to open '" STR_FMT "' for reading", STR_ARG(filename));
    ...
    {
        platform_fclose(f);
        str msg = str_fmt("failed to allocate memory for '" STR_FMT "' contents", STR_ARG(filename));
        return (SlurpFileResult)ERR(msg);
    }
    ...
    if (read != len)
    {
        platform_fclose(f);
        free(buf);
        str msg = str_fmt("failed to read '" STR_FMT "' contents", STR_ARG(filename));
        return (SlurpFileResult)ERR(msg);
    }
    ...
    return (SlurpFileResult)OK(str_acquire(buf, len));
}
