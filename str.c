...
{
    return (str){.ptr = s.ptr, .info = z_str_ref_info(str_len(s))};
}

str z_str_ref_chars(const char* ptr)
{
    return (str){.ptr = ptr, .info = z_str_ref_info(strlen(ptr))};
}
...
str str_acquire(const char* ptr, uint64_t len)
{
    ...
    return (str){.ptr = ptr, .info = z_str_owner_info(len)};
}
...
str str_ref_chars(const char* ptr, uint64_t len)
{
    ...
    return (str){.ptr = ptr, .info = z_str_ref_info(len)};
}
...
{
    return str_len(a) == str_len(b) && memcmp(a.ptr, b.ptr, str_len(a)) == 0;
}
